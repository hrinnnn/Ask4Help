...
CONFIG_NAME="libero_goal_ppo_openpi_pi05_sparse_runtime"
CONFIG_PATH="${RLINF_DIR}/examples/embodiment/config/${CONFIG_NAME}.yaml"
RUN_ID="$(timestamp)"
LOG="${GOAL_SPARSE_LOG:-/root/rlinf_pi05_libero_goal_sparse_${RUN_ID}.log}"
LOGDIR="${GOAL_SPARSE_LOGDIR:-${RLINF_DIR}/logs/libero_goal_sparse_${RUN_ID}}"
python "${SCRIPT_DIR}/write_runtime_config.py" \
  --output "${CONFIG_PATH}" \
  --task-suite libero_goal \
  --experiment-name "libero_goal_ppo_openpi_pi05_sparse" \
...
DEST="${RESULTS_DIR}/$(date +%Y%m%d_%H%M)_libero_goal_sparse_${GOAL_NUM_ENVS}env_${GOAL_MAX_STEPS}step"
mkdir -p "${DEST}/server_runtime_config"
cp -a "${LOG}" "${DEST}/"
cp -a "${LOGDIR}" "${DEST}/"
cp -a "${CONFIG_PATH}" "${DEST}/server_runtime_config/"
echo "Saved LIBERO Goal sparse result to ${DEST}"
