#!/usr/bin/env python3
"""Passive X-VLA airplane rollout with all internal and external detectors."""
...
def main() -> None:
        ...
        for episode_index in range(args.episodes):
            ...
            while len(executed) < args.max_episode_steps and not strict_success:
                ...
                for action in full_chunk[: args.execute_horizon]:
                    ...
                    ever_grasped |= bool_scalar(env.unwrapped.agent.is_grasping(env.unwrapped.obj))
                    strict_success |= bool_scalar(info.get("success", False))
                    if strict_success or bool_scalar(terminated) or bool_scalar(truncated):
                        break
            ...
            rows.append(row)
            print(
                f"[xvla-failure] split={args.split} episode={episode_index + 1}/{args.episodes} "
                f"seed={seed} grasp={int(ever_grasped)} strict={int(strict_success)}",
                flush=True,
            )
    finally:
        probe.close()
    ...
    summary = {
        "format": "xvla_airplane_failure_rollouts_v1",
