...
def main(args):
 ...
 for row in rows:
  ...
  report.update(status='PASS' if float(error.max())<1e-4 else 'REPLAY_DRIFT',max_qpos_error=float(error.max()),
   ...
   per_joint_max_error=np.max(np.abs(data['qpos'][start:start+len(expected)]-expected),axis=0).tolist(),warmup_replays=args.warmup_replays,native_observation=args.native_observation)
