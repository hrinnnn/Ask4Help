...
class PandaPosePlannerClient:
    def __init__(self):
        ...
        self.planner_mode = os.environ.get("PANDA_PLANNER_MODE", "screw_then_qpos")
        while True:
...
def _trajectory_cost(positions: np.ndarray, qpos: np.ndarray) -> float:
    ...
    return float(np.abs(deltas * weights).sum() + 0.05 * len(values))


def _move_to_best_pose(
    env,
    planner,
    targets,
    *,
    ...
    for target_world in targets:
        ...
        if positions is not None:
            candidates.append((_trajectory_cost(positions, qpos), target_world, positions))
    if not candidates:
        return False, 0
    _, target_world, positions = min(candidates, key=lambda item: item[0])
    for position in positions:
        env.step(np.concatenate([position[:7], [gripper]]).astype(np.float32))
    position_error = np.linalg.norm(
        _vector(base.agent.tcp.pose.p, 3) - np.asarray(target_world.p)
...
def _object_grasp_candidates(base, center: np.ndarray, closing: np.ndarray):
    mode = os.environ.get("PANDA_OBJECT_GRASP_MODE", "object_aligned")
    closings = (closing, -np.asarray(closing)) if mode == "symmetric_shortest" else (closing,)
    return [_top_down_grasp(base, center, candidate) for candidate in closings]
...
def solve_episode(env, seed: int, planner) -> dict[str, Any]:
    ...
    stages: dict[str, Any] = {
        ...
        "object_grasp_mode": os.environ.get("PANDA_OBJECT_GRASP_MODE", "object_aligned"),
    }
    ...
    object_grasps = _object_grasp_candidates(base, object_center, object_closing)
    reached, steps = _move_to_best_pose(
        env,
        planner,
        [grasp * sapien.Pose([0.0, 0.0, -0.140]) for grasp in object_grasps],
        gripper=1.0,
        position_tolerance=0.025,
    )
    stages["reached_object_pregrasp"] = reached
    stages["object_pregrasp_steps"] = steps
    reached, steps = _move_to_best_pose(env, planner, object_grasps, gripper=1.0)
