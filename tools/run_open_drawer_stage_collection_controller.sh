...
for old in "$SIDE/pids/pca_only_collection_v1.pid" "$SIDE/pids/diffdagger_collection_successful_id_v1.pid"; do
  while alive "$old"; do sleep 300; done
done
echo "$(date -Is) aggregate_side_collections_exited" >> "$LOG"
