...
def main(args):
 ...
 cached=[];features=[];targets=[];masks=[];means=[];input_features=[];timelines=[]
 ...
 for index,s in enumerate(samples):
  ...
  with torch.no_grad(),torch.autocast('cuda',dtype=torch.bfloat16):
   ...
   features.append(bridge.cpu().numpy());per_noise=[]
   ...
   cached.append(np.stack(per_noise));targets.append(action);masks.append(mask)
  if index%32==0:
 ...
 handle.remove()
 arrays=dict(head_features=np.asarray(cached),bridge_features=np.asarray(features),targets=np.asarray(targets),valid_mask=np.asarray(masks),head_weight=w,head_bias=b)
...
if __name__=='__main__':
 p=argparse.ArgumentParser();p.add_argument('--spec',type=Path,required=True);p.add_argument('--output',type=Path,required=True);p.add_argument('--xvla-root',default='/root/X-VLA-stackpyramid-clean');p.add_argument('--checkpoint',default='/root/xvla_stage2_inputs_priority/ckpt-7500');p.add_argument('--smoke',action='store_true');main(p.parse_args())
