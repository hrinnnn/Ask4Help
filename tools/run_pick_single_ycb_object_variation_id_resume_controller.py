...
def run_reload_smoke(smoke_checkpoint: Path) -> None:
    ...
    log = LOG_DIR / "id_resume_retry2_reload_forward_smoke.log"
    with log.open("w", encoding="utf-8") as stream:
...
def main() -> int:
    ...
    run_job("id_resume_smoke_from3500", RETRY / "smoke_2step", 3502, SOURCE, LOG_DIR / "id_resume_retry2_smoke.log")
    ...
    write_state(current_stage="id_training_resume_from3500_retry2", next_stage="id_checkpoint_probe_and_early_stop")
    run_job("id_sft_10000_retry2", TRAIN, 10000, SOURCE, LOG_DIR / "id_sft_formal_10000_retry2.log")
