...
def main() -> None:
    ...
    try:
        for position, index in enumerate(indices, start=1):
            ...
            action_mid_rows.append(
                features[f"action_expert_block_{action_mid_index:02d}"].detach().cpu().to(torch.float16)
            )
            vlm_mid_rows.append(
                features[f"vlm_block_{vlm_mid_index:02d}_mean"].detach().cpu().to(torch.float16)
            )
    ...
    payload = {
        ...
        "feature_semantics": {
            ...
            "action_expert_50": f"Action Expert block {action_mid_index} hidden state for ten action tokens",
            "vlm_50": f"mean valid VLM block {vlm_mid_index} prefix tokens",
        },
    ...
    manifest = {
        **{key: value for key, value in payload.items() if key not in {"fixed_prior", "statistics"}},
        ...
        "feature_shapes": {
            "bridge": list(bridge.shape),
            "action_expert_final": list(final.shape),
            "action_expert_50": list(action_mid.shape),
            "vlm_50": list(vlm_mid.shape),
        },
    }
