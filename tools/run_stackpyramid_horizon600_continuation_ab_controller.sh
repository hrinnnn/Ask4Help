...
BASE_ROOT="/mnt/data/ask4help/results/xvla_stackpyramid_oracle_repair_v3/continuation_50k_from_ckpt10000_lr1e-4_retry1/training"
DURABLE="/mnt/data/ask4help/results/xvla_stackpyramid_oracle_repair_v3/grasp_recovery_v1/horizon_600_20_id_891000_retry1"
WORK="/tmp/stackpyramid_horizon600_20_id_891000_retry1"
LOG="/root/ask4help_stage2_logs/stackpyramid_horizon600_20_id_891000_retry1.log"
...
declare -a NAMES=(continuation_ckpt40000 continuation_ckpt30000)
declare -a MODELS=("$BASE_ROOT/ckpt-40000" "$BASE_ROOT/ckpt-30000")
...
for INDEX in "${!NAMES[@]}"; do
  ...
  "$PY" "$ROOT/tools/evaluate_stackpyramid_xvla.py" \
    ...
    --split id --episodes 20 --start-seed 891000 --max-episode-steps 600 \
...
for row in summary.get("rows", []):
    metadata_max_steps[str(row.get("seed"))] = row.get("reset_metadata", {}).get("max_episode_steps")
    rows.append({
...
report = {
    ...
    "reset_metadata_max_episode_steps": metadata_max_steps,
    "rows": rows,
    "diagnostic_only": True,
}
Path(sys.argv[2]).write_text(json.dumps(report, indent=2) + "\n")
PY
  printf 'continuation horizon-600 diagnostic; formal 300-step gate unchanged\n' > "$DUR_OUT/HORIZON_600_DIAGNOSTIC_ONLY"
  rm -rf "$LOCAL_OUT"
done
...
if len(reports) != 2 or any(report["episodes"] != 20 for report in reports):
    raise SystemExit("incomplete continuation A/B horizon600 diagnostic")
summary = {
    "format": "stackpyramid_continuation_horizon600_ab_diagnostic_v1",
    "seed_manifest": "891000--891019",
...
(root / "horizon600_ab_summary.json").write_text(json.dumps(summary, indent=2) + "\n")
(root / "HORIZON_600_AB_COMPLETE").write_text("complete; diagnostic only\n")
PY
