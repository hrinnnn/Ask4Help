"""Device-selectable frozen X-VLA cache extraction for TNS utility diagnostics."""
...
def main(args):
 ...
 device=torch.device(args.device);torch.set_num_threads(args.threads);sys.path.insert(0,args.xvla_root)
 ...
 model=XVLA.from_pretrained(args.checkpoint,torch_dtype=torch.bfloat16).to(device).eval();model.requires_grad_(False);processor=XVLAProcessor.from_pretrained(args.checkpoint)
 ...
 cfg=dict(checkpoint=args.checkpoint,action_mode=model.action_mode,real_dim=8,num_actions=10,domain=0,loss_scale=100.,noise_times=[.25,.75],dtype='bf16 trunk, fp32 cached linear head',
  probe_scope='domain0 last action decoder; no checkpoint mutation or saving',device=str(device),torch_version=torch.__version__,image_size=224,image_channel_contract="historical cv2 BGR interpreted as PIL",threads=args.threads)
 ...
 language=processor.encode_language(['stack the red cube on the green cube'])['input_ids'].to(device)
 ...
 for index,s in enumerate(samples):
  ...
  example_saved=True
  image=torch.stack(images).unsqueeze(0).to(device).bfloat16();proprio=torch.zeros((1,20),device=device,dtype=torch.bfloat16);proprio[0,:9]=torch.tensor(t['state'][i],device=device,dtype=torch.bfloat16)
  ...
  target=torch.zeros((1,10,20),device=device,dtype=torch.bfloat16);target[0,:,:8]=torch.from_numpy(action).to(device).bfloat16();mask=np.arange(10)<length
  with torch.no_grad(),torch.autocast(device.type,dtype=torch.bfloat16):
   enc=model.forward_vlm(language,image,torch.ones((1,2),device=device,dtype=torch.bool))
   ...
   for mc,noise_t in enumerate([.25,.75]):
    generator=torch.Generator(device=device);generator.manual_seed(260906+index*17+mc)
    noise=torch.randn(target.shape,device=device,dtype=torch.bfloat16,generator=generator);noisy=noise*noise_t+target*(1-noise_t)
    pp,aa=model.action_space.preprocess(proprio,noisy)
    pred=model.transformer(domain_id=torch.zeros(1,device=device,dtype=torch.long),action_with_noise=aa,proprio=pp,t=torch.tensor([noise_t],device=device,dtype=torch.bfloat16),**enc)
   ...
   if args.free_action_probe:
    predictions=[]
    for mc in range(2):
     generator=torch.Generator(device=device);generator.manual_seed(270907+index*17+mc)
     prior=torch.randn(target.shape,device=device,dtype=torch.bfloat16,generator=generator);generated=torch.zeros_like(prior)
     for step in range(10,0,-1):
      noise_t=step/10;pp,aa=model.action_space.preprocess(proprio,prior*noise_t+generated*(1-noise_t))
      generated=model.transformer(domain_id=torch.zeros(1,device=device,dtype=torch.long),action_with_noise=aa,proprio=pp,t=torch.tensor([noise_t],device=device,dtype=torch.bfloat16),**enc)
     predictions.append(model.action_space.postprocess(generated)[0].float().cpu().numpy())
    free_predictions.append(predictions)
  if index%8==0:
   progress=dict(pid=os.getpid(),completed=index+1,total=len(samples),elapsed_seconds=time.time()-started,gpu_memory_gb=torch.cuda.max_memory_allocated()/1e9 if device.type=='cuda' else 0)
