...
if [[ -e "$OUTPUT_ROOT/FORMAL_COLLECTION_COMPLETE" ]]; then
    echo "refusing to reuse completed timing output: $OUTPUT_ROOT" >&2
    exit 2
fi
mkdir -p "$OUTPUT_ROOT"
...
printf '%s\n' 'complete' > "$OUTPUT_ROOT/FORMAL_COLLECTION_REMAINING_COMPLETE"
