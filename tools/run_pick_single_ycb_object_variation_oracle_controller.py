...
def run_gate(root: Path, run_root: Path, split: str, seed_start: int) -> int:
    ...
    output = run_root / "oracle_gate_v3" / split
    log_path = run_root / "logs" / f"oracle_gate_v3_{split}.log"
...
def main() -> None:
    ...
    write_state(
        ...
        authorized=True,
        current_stage="oracle_gate_v3_id",
        next_stage="oracle_gate_v3_ood",
    ...
    write_state(state_path, current_stage="oracle_gate_v3_ood", next_stage="oracle_gate_v3_complete")
    ...
    if ood_rc != 0:
        ...
        write_state(
            state_path,
            current_stage="oracle_gate_v3_ood_failed",
            next_stage="needs_user_decision_or_preapproved_recovery",
            terminal_marker="ORACLE_NOT_ACCEPTED",
        )
    ...
    (run_root / "ORACLE_GATE_PASSED").write_text(
        json.dumps({"id": "oracle_gate_v3/id", "ood": "oracle_gate_v3/ood"}, indent=2) + "\n",
        encoding="utf-8",
    )
    write_state(
        state_path,
        current_stage="oracle_gate_v3_complete",
