...
- 四组以 `B=2002` 个低层 expert actions 为名义上限；不得截断单条 suffix。正式流程在共同成功 seed 集合上计算不超过该上限的最大共同可达完整 suffix 预算，并把实际预算写入独立 contract、selection manifest 和训练元数据。retry2 的实际可达预算将在 collection 完成后解析，不能事后假定为 2002。首轮 400 条 screen 中有 286 条满足 clean stage-localized failure 条件，因此 retry2 使用其中确定性选出的 250 条共同 cohort，不改变 OOD 定义或专家预算规则。
