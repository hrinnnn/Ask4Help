...
补充对照：在同一 `ckpt-10000`、同一 ID seed 上，`execute_horizon=1/5/10` 的独立诊断均为 `0/1`，且 `mug_parked` 与 `sphere_grasped` 均为 0；因此当前失败不能归因于单一的 5-step rollout 节拍。对应输出保存在 `diagnostics/control_horizon_ckpt10000_v2/`，仅作诊断，不进入正式结果。
