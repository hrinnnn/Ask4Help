...
- `diagnostic_partial_evaluation_2026-08-26`: 用户已授权在正式 Stage-B utility evaluation 前先做独立部分评估。GPU4 已独立核验为空闲，控制器改为立即运行（`--start-now`，CPU20-39），不等待 Stage-B 完成；它使用两个已完成的 knee checkpoint（StackCube/Airplane `step_20/seed_17001`），每个 task 做 20-ID + 20-OOD，使用独立 seed `190000--191119` 和独立输出根 `diagnostic_partial_evaluation_v1`。该诊断只验证 evaluator、checkpoint reload 和初步成功率，不计入正式 100-episode 分母，也不改变正式阈值、seed 或训练协议；Stage-B 总控 PID `2926040` 在 `PARTIAL_EVAL_COMPLETE` 前不会启动正式评估。
...
- `current_stage`: `ood_serial_retry3_handle_offline_oracle`; Handle/PCA (`3/97`), Handle/DiffDAgger (`23/77`), and Handle/Failure-Recovery (`48/52`) each reached 100 accepted with independent artifact audits. Protocol PID `2561352` now runs the sole Handle/Offline-Oracle collector on GPU0 (latest live count `12/12`); the earlier Failure-Recovery calibration mismatch parts remain diagnostics, and the filtered 18-detector adapter is recorded under clean retry3 assets.
