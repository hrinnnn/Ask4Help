...
- `next_stage`: run a fresh weights-only 2-step smoke from complete `global_step_3500` with a reset optimizer and cumulative step offset, then resume ID training in `formal_10000_retry5`
...
- `runtime_recovery`: `/data` was re-mounted from the existing `/dev/sdb1`; the original run root was re-audited on `2026-08-24 11:11+08:00`. No controller was alive, `global_step_3500` is complete, and the old retry1 `global_step_4000` contains only an incomplete DCP shard without `full_weights.pt`; retry2 is preserved as a Ray socket-path diagnostic, retry3 as a scheduler-horizon non-finite-loss diagnostic, and retry4 as an optimizer-resume non-finite-loss diagnostic. The active resume uses new `id_training_v1/formal_10000_retry5` with weights-only loading from `global_step_3500`, reset optimizer/scheduler, cumulative `initial_step=3500`, and short `/sdd` Ray/TMP roots. Old retry1/2/3/4 and the mount-recovery diagnostic remain untouched.
