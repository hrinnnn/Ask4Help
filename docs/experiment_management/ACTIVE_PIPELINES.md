...
- `next_stage`: `stage_b_evaluation_then_stage_c_passive_then_gate_data_training_utility_reconciliation`
...
- `source_commit`: `41220e2`; local branch `codex/xvla-fixed-grid-knee`; server worktree is detached at the same commit (diagnostic partial-evaluation gate recorded)
...
- `resource_preflight_2026-08-25`: calibration and smoke evidence passed; formal Stage-B is running on the explicitly selected 5090 GPU5/CPU `0-19`. Other protected GPUs and the H20 owner are untouched. Training controller PID `4087857`; total supervisor PID `2926040`; diagnostic partial-evaluation controller PID `2941279`; supervisor interval `900s`.
...
- `diagnostic_partial_evaluation_2026-08-26`: 用户已授权在正式 Stage-B utility evaluation 前先做独立部分评估。控制器 PID `2941279` 已持久化启动并排队等待 `STAGE_B_TRAINING_COMPLETE`，随后只在 GPU5 安全释放时运行两个已完成的 knee checkpoint（StackCube/Airplane `step_20/seed_17001`），每个 task 做 20-ID + 20-OOD，使用独立 seed `190000--191119` 和独立输出根 `diagnostic_partial_evaluation_v1`。该诊断只验证 evaluator、checkpoint reload 和初步成功率，不计入正式 100-episode 分母，也不改变正式阈值、seed 或训练协议；Stage-B 总控 PID `2926040` 在 `PARTIAL_EVAL_COMPLETE` 前不会启动正式评估。
...
- `current_stage`: `ood_serial_retry3_handle_failure_recovery`; Handle/PCA completed with artifact reconciliation (ID/OOD=3/97), Handle/DiffDAgger completed with the repaired cleanup-aware controller, and protocol PID `2561352` now runs the sole Failure-Recovery collector from `part_003` on GPU0. `part_001` (PCA-calibration key mismatch) and `part_002` (full-calibration schema mismatch) remain diagnostics; the filtered 18-detector calibration adapter is recorded under the clean retry3 assets.
