...
- `current_stage`: `both_task_calibration_complete_budget_resolution_failed`
- `next_stage`: `user_decision_more_seeds_or_budget_fallback_before_stage_b`
...
- `source_commit`: `b9ef07b`; local branch `codex/xvla-fixed-grid-knee`; GitHub branch pushed
...
- `current_stage`: `oracle_handle_ood_running`; retry6/8/9/10 are preserved as Oracle diagnostics/rejections. Retry8/9/10 measured Grasp OOD `17/20`; retry10 also exposed that OOD reset metadata was hardcoded to `split=id`, so it is not canonical. Retry7 remains `ENGINEERING_ORACLE_SHARED_PLANNER_SERIALIZATION_DIAGNOSTIC`. Retry11 controller PID `632609` runs the three splits serially on GPU5 with output on `/data`; live reset split now comes from `env.unwrapped.rlinf_split`, and same-seed planner/replay retry is reference up to 3/replay up to 2. Failure-detection waiter PID `632610` and OOD collection/training waiter PID `632611` are active and will only advance after their explicit completion markers.
