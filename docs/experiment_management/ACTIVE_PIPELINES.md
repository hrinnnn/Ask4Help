...
- `next_stage`: run a fresh 2-step resume/reload smoke from complete `global_step_3500` with the original 10000-step scheduler horizon, then resume ID training in `formal_10000_retry4`
...
- `runtime_recovery`: `/data` was re-mounted from the existing `/dev/sdb1`; the original run root was re-audited on `2026-08-24 11:11+08:00`. No controller was alive, `global_step_3500` is complete, and the old retry1 `global_step_4000` contains only an incomplete DCP shard without `full_weights.pt`; retry2 is preserved as a Ray socket-path diagnostic and retry3 as a non-finite-loss scheduler-horizon diagnostic. The active resume uses new `id_training_v1/formal_10000_retry4` with short `/sdd` Ray/TMP roots and the original 10000-step scheduler horizon. Old retry1/2/3 and the mount-recovery diagnostic remain untouched.
