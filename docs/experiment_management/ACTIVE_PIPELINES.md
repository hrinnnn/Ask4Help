...
- `source_commit`: `e656e55`; local branch `codex/xvla-fixed-grid-knee`; server worktree is detached at the same commit (diagnostic partial-evaluation result recorded)
...
- `current_stage`: `ood_serial_retry3_grasp_pca_only`; all four Handle methods reached 100 accepted with independent artifact audits: PCA `3/97`, DiffDAgger `23/77`, Failure-Recovery `48/52`, Offline-Oracle `0/100`. Protocol PID `2561352` now runs the sole Grasp/PCA collector on GPU0 (latest live count `31/1`); earlier calibration mismatch parts remain diagnostics, and the filtered 18-detector adapter is recorded under clean retry3 assets.
